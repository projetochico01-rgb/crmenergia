...
function LeadWorkspace({ lead, profiles, currentUserId, currentRole, details, loading, tab, setTab, onEdit, onAssign }: { lead: LeadsPipeline; profiles: CrmUserProfile[]; currentUserId: string; currentRole: 'admin' | 'atendente'; details: Details; loading: boolean; tab: DetailTab; setTab: (tab: DetailTab) => void; onEdit: () => void; onAssign: (userId: string | null) => void }) {
  ...
  async function queueConversion(eventName: 'LeadQualified' | 'Purchase') { let value = Number(lead.closed_value ?? lead.value ?? 0); let closedAt = new Date().toISOString().slice(0, 10); if (eventName === 'Purchase') { const enteredValue = window.prompt('Valor da venda:', String(value)); if (enteredValue === null) return; value = Number(enteredValue); const enteredDate = window.prompt('Data da venda (AAAA-MM-DD):', closedAt); if (enteredDate === null) return; closedAt = enteredDate; } const response = await fetch('/api/admin/conversions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lead_id: lead.id, event_name: eventName, value, closed_at: closedAt }) }); const payload = await response.json(); setConversionFeedback(response.ok ? 'Evento preparado. O envio real para a Meta permanece desligado.' : payload.error ?? 'Evento não preparado.'); }
  ...
  return <div className="space-y-4 p-5">
    ...
    <div className="flex flex-wrap items-center gap-2"><button onClick={onEdit} className="button-secondary"><Pencil className="h-4 w-4" /> Editar</button>{!lead.assigned_user_id && <button onClick={() => onAssign(currentUserId)} className="button-primary"><UserCheck className="h-4 w-4" /> Assumir lead</button>}{lead.assigned_user_id && (currentRole === 'admin' || lead.assigned_user_id === currentUserId) && <button onClick={() => onAssign(null)} className="button-secondary"><RotateCcw className="h-4 w-4" /> Fila livre</button>}{currentRole === 'admin' && <><select value={lead.assigned_user_id ?? ''} onChange={(event) => onAssign(event.target.value || null)} className="control max-w-60"><option value="">Fila livre</option>{profiles.map((profile) => <option key={profile.user_id} value={profile.user_id}>{profile.display_name}</option>)}</select><button disabled={lead.consent_status !== 'granted'} onClick={() => void queueConversion('LeadQualified')} className="button-secondary">Marcar como qualificado</button><button disabled={lead.consent_status !== 'granted'} onClick={() => void queueConversion('Purchase')} className="button-secondary">Registrar venda</button></>}</div>
    {conversionFeedback && <p role="status" className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-3 text-xs text-amber-100">{conversionFeedback}</p>}
